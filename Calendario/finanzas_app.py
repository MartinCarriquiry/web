# finanzas_app.py
# ---------------------------------------------------------
# Finanzas personales con Streamlit + Supabase (Auth + DB)
# - Cliente de Supabase por sesión (NO cache global)
# - Login/Registro en la propia app
# - Caché de datos por usuario (clave: user_id)
# - Manejo seguro cuando no hay datos
# - Admin de categorías (alta/baja + reasignación)
# - Alta de movimientos
# - KPIs + gráfico de balance
# ---------------------------------------------------------
    ...
    with tab_login:
        ...
        if st.button("Ingresar"):
            try:
                ...
                st.session_state.pop("supa", None)  # refresco del token en el cliente
...
def load_categories(user_id: str) -> pd.DataFrame:
    ...
    df = pd.DataFrame(res.data or [], columns=["id", "user_id", "name", "kind"])
    return df
...
def ensure_default_categories(user_id: str):
    """Si el usuario no tiene categorías, creamos unas mínimas."""
...
def delete_category(user_id: str, name: str, reassign_to: str | None):
    supa = get_supabase()
    # Reasignación previa si corresponde
    if reassign_to:
...
def app_view(user):
    ...
    cats_opts = cats_df.loc[cats_df["kind"] == kind, "name"].tolist()
    if not cats_opts:
        cats_opts = ["Sin categoría"]
    ...
    with st.sidebar.expander("⚙️ Administrar categorías", expanded=False):
        ...
        count_assoc = 0
        if not tx_df.empty and sel_del:
            count_assoc = int((tx_df["category"] == sel_del).sum())
    ...
    today = date.today()
    btn_hoy = colp1.button("Hoy")
    btn_mes = colp2.button("Este mes")
    btn_anio = colp3.button("Este año")
    btn_rango = colp4.button("Rango")
    ...
    if btn_hoy:
        st.session_state.rng = (today, today)
    if btn_mes:
        st.session_state.rng = (today.replace(day=1), today)
    if btn_anio:
        st.session_state.rng = (date(today.year, 1, 1), today)
    ...
    if fdf.empty:
        ...
        # serie de balance acumulado por fecha
        s = (
