    ...
    with tab2:
        ...
        if st.button("Crear cuenta", use_container_width=True):
            try:
                ...
                st.success("Cuenta creada. Revisá tu email si pide verificación.")
            except Exception as e:
...
def ensure_defaults():
    ...
    rows = [{"user_id": user_id, "name": n, "kind": k} for n,k in defaults if n not in have]
    if rows:
        supa.table("categories").insert(rows).execute()

@st.cache_data(ttl=30)
def load_df():
    res = supa.table("transactions").select("*").eq("user_id", user_id).order("tdate", desc=True).execute()
    df = pd.DataFrame(res.data or [])
    if not df.empty:
        df["tdate"] = pd.to_datetime(df["tdate"])
    return df

def list_categories(kind=None) -> pd.DataFrame:
    q = supa.table("categories").select("name,kind").eq("user_id", user_id)
    if kind: q = q.eq("kind", kind)
    res = q.order("name").execute()
    return pd.DataFrame(res.data or [])

def add_category(name, kind):
    supa.table("categories").insert({"user_id": user_id, "name": name.strip(), "kind": kind}).execute()
    st.cache_data.clear()
...
def add_transaction(tdate, amount, kind, category, note):
...
def balance_series(full_df: pd.DataFrame, freq: str = "D") -> pd.DataFrame:
    ...
    tmp = full_df.copy()
    tmp["sign"] = tmp.apply(lambda r: r["amount"] if r["kind"]=="ingreso" else -r["amount"], axis=1)
    if freq == "D":
...
kind = c2.selectbox("Tipo", ["ingreso","gasto","inversion"], index=1)
...
if st.sidebar.button("Agregar", type="primary", use_container_width=True):
    if amount > 0 and category:
        add_transaction(tdate, amount, kind, category, note)
...
with st.sidebar.expander("⚙️ Administrar categorías"):
    ...
    if st.button("Guardar categoría"):
        if new_name.strip():
            add_category(new_name, new_kind)
...
if not fdf.empty:
    show = ["id","tdate","kind","category","amount","note"]
